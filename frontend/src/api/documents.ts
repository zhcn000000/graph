...
import type { DocumentUploadResponse, FileIngestResponse, SearchRequest, SearchResponse } from "./types";
...
export const ingestArtifacts = (museum?: string, limit?: number): Promise<FileIngestResponse> => {
  ...
  return request.post(
    `/rag/documents/ingest-artifacts?${new URLSearchParams(params as Record<string, string>).toString()}`,
  );
};
