...
export const getNeighbors = (uri: string, direction?: string, maxHops?: number): Promise<GraphOperationResponse> => {
...
export const findPaths = (startUri: string, endUri: string, maxHops?: number): Promise<GraphOperationResponse> => {
  return request.get(`/graph/paths/${encodeURIComponent(startUri)}/${encodeURIComponent(endUri)}`, {
    max_hops: maxHops,
  });
};
