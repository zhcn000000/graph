...
export default function ChatPage() {
    ...
    async (command: string, item: ConversationItem) => {
      ...
      if (command === 'rename') {
        const newName = await new Promise<string | null>((resolve) => {
          ...
          Modal.confirm({
            title: '重命名会话',
            content: <Input defaultValue={item.label} autoFocus onChange={(e) => { value = e.target.value }} />,
            okText: '确定',
            cancelText: '取消',
            onOk: async () => {
              if (!value.trim()) { message.warning('会话名称不能为空'); throw new Error('invalid') }
              resolve(value.trim())
            },
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (hasUploadingAttachments) { message.warning('文件上传中，请稍后发送'); return }
    if (!activeConversationKey) { message.warning('缺少会话信息，无法发送消息'); return }
  ...
  const handleAttachmentUpload: UploadProps['customRequest'] = async (options) => {
    ...
    if (!(file instanceof File)) { onError?.(new Error('文件格式错误')); return }
    const uid = createAttachmentId('upload')
    const uploadItem: UploadFile = { uid, name: file.name, status: 'uploading', originFileObj: file as UploadFile['originFileObj'] }
  ...
  const bubbleItems = useMemo(() => {
          ...
          if (toolItems.length) {
            const thoughtItems: ThoughtChainItemType[] = toolItems.map((tool) => ({
              ...
              content: (
                <Flex>
                  {tool.args ? (
                    <ThoughtChain.Item variant="solid" title="调用工具" description={typeof tool.args === 'string' ? tool.args : JSON.stringify(tool.args)} />
                  ) : null}
                  {tool.result ? (
                    <ThoughtChain.Item variant="solid" title="工具结果" description={typeof tool.result === 'string' ? tool.result : JSON.stringify(tool.result)} />
                  ) : null}
                </Flex>
              ),
              collapsible: true,
    ...
    return messageInfos.map((info) => {
      ...
      const actionsItems: ItemType[] = [
        { key: 'copy', actionRender: <Actions.Copy text={copyText} /> },
      ]
      return {
        key: String(info.id),
        role: item.role,
        ...
        <Main>
          ...
          <Sender
                    ...
                    <Attachments
                      ...
                      placeholder={(type) =>
                        type === 'drop'
                          ...
                          : { icon: <CloudUploadOutlined />, title: '在此处上传文件', description: '单个文件不超过10MB' }
                      }
                    />
                  </AttachmentPanel>
                </Sender.Header>
            ...
            prefix={
              <SenderPrefix>
                <Tooltip title="附件上传">
                  <Sender.Switch icon={<PaperClipOutlined />} value={attachmentVisible} onChange={setAttachmentVisible} disabled={isRequesting} />
                </Tooltip>
                <Tooltip title="工具选择">
                  <Sender.Switch icon={<ToolOutlined />} value={toolVisible} onChange={setToolVisible} disabled={isRequesting} />
                </Tooltip>
                <Tooltip title="深度思考">
