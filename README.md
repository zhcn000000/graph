...
3、知识问答子系统本子系统基于知识图谱数据与大语言模型，为用户提供自然语言形式的文物知识问答服务，支持从简单属性查询到复杂关系推理的多层次问答能力。建议采用"知识图谱+大语言模型"的检索增强生成（RAG）架构：用户输入自然语言问题；系统进行意图识别与实体抽取，将问题转化为图数据库查询语句（SPARQL或Cypher）；从知识图谱中检索相关事实与数据；将检索结果作为上下文传入大语言模型，生成自然流畅的回答；返回答案并标注数据来源。该架构既保证了答案的事实准确性（来自知识图谱），又具备良好的语言表达能力（来自大语言模型），有效避免大模型幻觉问题。（1）简单问答支持单实体单属性的问答，至少覆盖以下10类问题：文物收藏地：该文物现藏于哪家博物馆？文物年代：该文物属于哪个历史时期？文物材质：该文物由什么材料制成？文物类型：该文物属于哪种器物类别？文物介绍：请介绍一下这件文物。书画作者：该书画作品的作者是谁？针对书画类文物。作者生平：该作者的生平经历是怎样的？针对书画类文物。同一作者作品：该作者还有哪些作品被海外博物馆收藏？ 同一朝代文物：某朝代有哪些代表性文物？文物尺寸与规格：该文物的尺寸和重量是多少？相关文物推荐：与该文物风格或主题相似的文物有哪些？技术要求：对于知识图谱中不存在的问题，应明确告知用户"暂无相关数据"，而非生成不实答案。（2）答案溯源与可信度展示为增强问答系统的可信度，每条回答须附带数据来源说明：标注答案来源（如"数据来源：克利夫兰艺术博物馆"）；提供指向原始数据详情页的链接；对于由大语言模型生成的补充性描述，须与知识图谱事实性内容明确区分标注。（3）多轮对话（选做）支持基于上下文的连续追问，提升问答交互的自然性：系统能够理解代词指代，例如用户先问"青花瓷是什么？"后问"它主要产于哪个朝代？"，系统能正确理解"它"指代青花瓷；保留最近5轮对话上下文用于意图理解；支持用户主动切换话题，系统能识别并重置上下文。（4）复杂问答（选做）支持需要多步推理或跨实体关联的复杂问题，主要包括：多跳推理问答：例如"收藏青铜器最多的博物馆是哪家？该博物馆在哪个城市？"比较类问答：例如"唐代和宋代的瓷器在风格上有何不同？"统计类问答：例如"大都会博物馆共收藏了多少件中国文物？"路径查询：例如"某件文物与某位历史人物之间有什么关联？"技术建议：可基于知识图谱的多跳查询（如SPARQL路径查询）结合大语言模型进行推理链生成。（5）问答质量反馈机制（选做）建立用户反馈，持续优化问答质量：每条回答下方提供"有帮助"与"不准确"两个反馈按钮；对于用户标记为不准确的回答，记录至问题日志供人工审核；统计高频未能正确回答问题类型，为知识图谱补充与模型优化提供依据。（6）闲聊与文化导览（选做）支持与用户进行文物文化相关主题的自由对话：能够介绍中国各历史朝代的文化背景与艺术特色；能够讲解中国传统工艺（如陶瓷烧制、书画技法等）的基础知识；支持根据用户兴趣推荐相关文物或拓展阅读内容；对于超出文物文化领域的话题，礼貌告知用户系统专注范围。
