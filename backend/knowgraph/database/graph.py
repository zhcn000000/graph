...
class GraphStore:
    ...
    def create_vertex(self, label: str, properties: dict[str, Any]) -> Vertex | None:
        import asyncio

        return asyncio.get_event_loop().run_until_complete(self.graph_manager.acreate_vertex(label, properties))

    def get_vertex(self, uri: str, label: str | None = None) -> Vertex | None:
        import asyncio

        return asyncio.get_event_loop().run_until_complete(self.graph_manager.aget_vertex(uri, label))

    def update_vertex(self, uri: str, properties: dict[str, Any], label: str | None = None) -> Vertex | None:
        import asyncio

        return asyncio.get_event_loop().run_until_complete(self.graph_manager.aupdate_vertex(uri, properties, label))

    def delete_vertex(self, uri: str, label: str | None = None) -> bool:
        import asyncio

        return asyncio.get_event_loop().run_until_complete(self.graph_manager.adelete_vertex(uri, label))
    ...
    def get_neighbors(self, uri: str, direction: str = "both", max_hops: int = 1) -> list[Vertex]:
        ...
        return asyncio.get_event_loop().run_until_complete(self.graph_manager.aget_neighbors(uri, direction, max_hops))

    def execute_cypher(self, cypher: str, params: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        import asyncio

        return asyncio.get_event_loop().run_until_complete(self.graph_manager.aexecute_cypher(cypher, params))

    def query_by_type(self, entity_type: str, limit: int = 100) -> list[Vertex]:
        import asyncio

        return asyncio.get_event_loop().run_until_complete(self.graph_manager.aquery_by_type(entity_type, limit))

    def traverse(self, start_uri: str, max_hops: int = 3, direction: str = "both") -> GraphPath:
        import asyncio

        return asyncio.get_event_loop().run_until_complete(self.graph_manager.atraverse(start_uri, max_hops, direction))

    def find_paths(self, start_uri: str, end_uri: str, max_hops: int = 5) -> list[GraphPath]:
        import asyncio

        return asyncio.get_event_loop().run_until_complete(self.graph_manager.afind_paths(start_uri, end_uri, max_hops))

    def create_graph(self) -> bool:
        import asyncio

        return asyncio.get_event_loop().run_until_complete(self.graph_manager.acreate_graph())

    def drop_graph(self) -> bool:
        import asyncio

        return asyncio.get_event_loop().run_until_complete(self.graph_manager.adrop_graph())
